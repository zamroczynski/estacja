    ...
    <body id="page-top" class="bg-light d-flex flex-column min-vh-100">
        <!-- Navigation-->
            <nav class="navbar navbar-expand-lg bg-secondary" id="mainNav">
                <div class="container">
                    <a class="navbar-brand me-1" href="@auth{{ route('home') }}@endauth"><span class="text-success">e</span>STACJA</a>
                    @auth
                    @if(Session::get('notificationsAmount') > 0)
                        <a href="{{ route('notifications') }}" class="text-white ms-5">
                            <i class="far fa-bell fa-2x position-relative">
                                <span class="position-absolute border-1 top-0 start-100 translate-middle badge rounded-pill bg-danger fs-6">
                    ...
                    @empty(Session::get('notificationsAmount'))
                        <a href="{{ route('notifications') }}" class="text-light"><i class="far fa-bell-slash fa-lg ms-5"></i></a>
                    @endempty
                    @endauth
            ...
            <footer class="mt-auto">
                <div class="copyright py-4 text-center text-white">
                    <div class="container">
                        <small>&copy; eStacja version {{ config('app.ver') }} by Damian Zamroczynski - 2022 - kontakt: damian.zamroczynski@gmail.com</small>
                    </div>
                </div>
            </footer>
